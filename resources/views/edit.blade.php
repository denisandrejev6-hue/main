    ...
    <form action="{{ route('pasakumi.update', $data->ID) }}" method="POST" style="max-width:800px;">
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:16px; margin-bottom:16px;">
            ...
            <div class="form-control">
                ...
                <input type="time" name="sakuma_laiks" id="sakuma_laiks" value="{{ old('sakuma_laiks', $data->sakuma_laiks) }}" style="width:90%; padding:10px; border-radius:6px;" onchange="validateTimeRange()">
            ...
            <div class="form-control">
                ...
                <input type="time" name="beigu_laiks" id="beigu_laiks" value="{{ old('beigu_laiks', $data->beigu_laiks) }}" style="width:90%; padding:10px; border-radius:6px;" onchange="validateTimeRange()">
            ...
            <div class="form-control">
                ...
                <select name="darbinieks_id" style="width:90%; padding:10px; border-radius:6px;">
                    ...
                    @foreach($darbinieki as $d)
                        <option value="{{ $d->ID }}" {{ old('darbinieks_id', $data->darbinieks_id) == $d->ID ? 'selected' : '' }}>{{ $d->vards }} ({{ $d->ID }})</option>
                    @endforeach
                </select>
            </div>
            <div class="form-control">
                ...
                <select name="telpa_id" style="width:90%; padding:10px; border-radius:6px;">
                    ...
                    @foreach($telpas as $t)
                        <option value="{{ $t->ID }}" {{ old('telpa_id', $data->telpa_id) == $t->ID ? 'selected' : '' }}>{{ $t->nosaukums }} ({{ $t->ID }})</option>
                    @endforeach
                </select>
            </div>
        </div>
