                        ...
                        <div style="padding:20px;">
                            ...
                            <div style="margin-bottom:16px;">
                                <p style="margin:8px 0; color:#666;"><strong style="color:#333;">Datums:</strong> 
                                    {{ \Carbon\Carbon::parse($item->datums_no)->format('d.m.Y') }}
                                    @if($item->datums_lidz && $item->datums_lidz != $item->datums_no)
                                        - {{ \Carbon\Carbon::parse($item->datums_lidz)->format('d.m.Y') }}
                                    @endif
                                </p>
                                <p style="margin:8px 0; color:#666;"><strong style="color:#333;">Laiks:</strong> 
                                    {{ $item->sakuma_laiks ?? '00:00' }} - {{ $item->beigu_laiks ?? '00:00' }}
                                </p>
                            </div>
                ...
                <table style="width:100%; border-collapse:collapse; font-size:14px;">
                    <thead>
                        <tr style="background:#f5f5f5; border-bottom:2px solid #ddd;">
                            ...
                            <th style="padding:16px; text-align:left; color:#333; font-weight:600;">Datums no</th>
                            <th style="padding:16px; text-align:left; color:#333; font-weight:600;">Datums līdz</th>
                    ...
                    <tbody>
                        @foreach($data as $item)
                            <tr style="border-bottom:1px solid #eee; transition:background 0.2s;" onmouseover="this.style.background='#f9f9f9'" onmouseout="this.style.background='white'">
                                ...
                                <td style="padding:16px;">{{ $item->datums_no ? \Carbon\Carbon::parse($item->datums_no)->format('d.m.Y') : '—' }}</td>
                                <td style="padding:16px;">{{ $item->datums_lidz ? \Carbon\Carbon::parse($item->datums_lidz)->format('d.m.Y') : '—' }}</td>
                                <td style="padding:16px; font-weight:500; color:#4CAF50;">{{ $item->sakuma_laiks ?? '—' }}</td>
                                <td style="padding:16px; font-weight:500; color:#f44336;">{{ $item->beigu_laiks ?? '—' }}</td>
                                ...
                                <td style="padding:16px;">
                                    @if($item->darbinieks)
                                        <span style="background:#e3f2fd; color:#1976D2; padding:4px 8px; border-radius:4px; font-size:12px;">
                                            {{ $item->darbinieks->vards ?? 'Nav vārda' }}
                                        </span>
                                    @else
                                ...
                                <td style="padding:16px;">
                                    @if($item->telpa)
                                        <span style="background:#fff3e0; color:#f57c00; padding:4px 8px; border-radius:4px; font-size:12px;">
                                            {{ $item->telpa->nosaukums ?? 'Nav nosaukuma' }}
                                        </span>
                                    @else
                                        <span style="color:#999;">—</span>
                                    @endif
